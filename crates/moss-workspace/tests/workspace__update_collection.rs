...
async fn rename_collection_success() {
    let (_ctx, _workspace_path, mut workspace, services, cleanup) = setup_test_workspace().await;
...
async fn rename_collection_empty_name() {
    let (_ctx, _workspace_path, mut workspace, _services, cleanup) = setup_test_workspace().await;
...
async fn update_collection_new_icon() {
    let (_ctx, workspace_path, mut workspace, services, cleanup) = setup_test_workspace().await;
    let collection_name = random_collection_name();
    let id = workspace
        .create_collection(&CreateCollectionInput {
