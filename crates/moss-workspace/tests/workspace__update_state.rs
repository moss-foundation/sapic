...
use moss_workspace::{
    ...
    storage::segments::{LAYOUT_PANEL_SEGKEY, LAYOUT_SIDEBAR_SEGKEY},
};
...
async fn update_state_sidebar_part() {
    ...
    // Check position
    let position_value =
        GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("position")).unwrap();
    let stored_position: SidebarPosition = position_value.deserialize().unwrap();
    assert_eq!(stored_position, SidebarPosition::Left);

    // Check size
    let size_value = GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("size")).unwrap();
    let stored_size: usize = size_value.deserialize().unwrap();
    assert_eq!(stored_size, 250);

    // Check visible
    let visible_value =
        GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("visible")).unwrap();
...
async fn update_state_panel_part() {
    ...
    // Check size
    let size_value = GetItem::get(item_store.as_ref(), LAYOUT_PANEL_SEGKEY.join("size")).unwrap();
    let stored_size: usize = size_value.deserialize().unwrap();
    assert_eq!(stored_size, 200);

    // Check visible
    let visible_value =
        GetItem::get(item_store.as_ref(), LAYOUT_PANEL_SEGKEY.join("visible")).unwrap();
    let stored_visible: bool = visible_value.deserialize().unwrap();
...
async fn update_state_multiple_updates() {
    ...
    let sidebar_position: SidebarPosition =
        GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("position"))
            .unwrap()
            .deserialize()
            .unwrap();
    assert_eq!(sidebar_position, SidebarPosition::Left);

    let sidebar_size: usize = GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("size"))
        .unwrap()
        .deserialize()
        .unwrap();
    ...
    let sidebar_visible: bool =
        GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("visible"))
            .unwrap()
            .deserialize()
            .unwrap();
    ...
    // Check panel values
    let panel_size: usize = GetItem::get(item_store.as_ref(), LAYOUT_PANEL_SEGKEY.join("size"))
        .unwrap()
        .deserialize()
        .unwrap();
    ...
    let panel_visible: bool =
        GetItem::get(item_store.as_ref(), LAYOUT_PANEL_SEGKEY.join("visible"))
            .unwrap()
            .deserialize()
            .unwrap();
    ...
    let updated_sidebar_size: usize =
        GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("size"))
            .unwrap()
            .deserialize()
            .unwrap();
    ...
    let panel_size_after: usize =
        GetItem::get(item_store.as_ref(), LAYOUT_PANEL_SEGKEY.join("size"))
            .unwrap()
            .deserialize()
            .unwrap();
...
async fn update_state_overwrite_existing() {
    ...
    let storage_service = services.get::<StorageService>();
    let item_store = storage_service.__storage().item_store();
    let initial_size: usize = GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("size"))
        .unwrap()
        .deserialize()
        .unwrap();
    ...
    let updated_size: usize = GetItem::get(item_store.as_ref(), LAYOUT_SIDEBAR_SEGKEY.join("size"))
        .unwrap()
        .deserialize()
        .unwrap();
