        ...
        let new_handle = tokio::spawn(async move {
            let result = match handle.await {
                ...
                Err(join_error) => {
                    ...
                    Err(joinerror::Error::new::<Unknown>(format!(
                        "Task join error: {:?}",
                        join_error
                    )))
                }
...
mod tests {
    ...
    async fn test_task_ready_error() {
        let error = joinerror::Error::new::<Unknown>("test error");
        let task: Task<i32> = Task::ready(Err(error));
        let result = task.await;
        assert!(result.is_err());
        assert!(result.unwrap_err().is::<Unknown>());
    }
    ...
    async fn test_detach_log_if_err_error() {
        ...
        let task: Task<i32> = Task::with_context(&ctx, async {
            sleep(Duration::from_millis(10)).await;
            Err(joinerror::Error::new::<Unknown>("test error"))
        });
