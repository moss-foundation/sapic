...
use moss_collection::models::operations::{CreateRequestGroupInput, CreateRequestInput, RenameRequestGroupInput};
...
async fn rename_request_group_success() {
    ...
    let rename_request_group_result = collection.rename_request_group(
        RenameRequestGroupInput {
            path: PathBuf::from(&old_name),
    ...
    let old_path = collection_path.join(
        request_group_relative_path(Path::new(&old_name))
    );
    let new_path = collection_path.join(
        request_group_relative_path(Path::new(&new_name))
    );
...
async fn rename_request_group_empty_name() {
    ...
    assert!(matches!(rename_request_group_result, Err(OperationError::Validation { .. })));
...
async fn rename_request_group_special_chars() {
    ...
    for char in FOLDERNAME_SPECIAL_CHARS {
        ...
        let old_path = collection_path.join(
            request_group_relative_path(Path::new(&current_name))
        );
        let expected_path = collection_path.join(
            request_group_relative_path(Path::new(&new_request_group_name))
        );
        assert!(!old_path.exists());
...
async fn rename_request_group_subfolder() {
    ...
    let rename_request_group_output = collection.rename_request_group(
        RenameRequestGroupInput {
            path: PathBuf::from(&request_group_name).join("subfolder"),
    ...
    let new_path = collection_path.join(
        request_group_relative_path(&Path::new(&request_group_name).join("subfolder_new"))
    );
    ...
    assert_eq!(requests.0.len(), 2);
    assert!(requests.0.iter().any(
        |request| request.name == "outer_request" &&
            request.relative_path_from_requests_dir ==
                PathBuf::from(&request_group_name)
                    .join(request_folder_name("outer_request"))
    ));
    assert!(requests.0.iter().any(
        |request| request.name == "inner_request" &&
            request.relative_path_from_requests_dir ==
                PathBuf::from(&request_group_name)
                    .join("subfolder_new")
