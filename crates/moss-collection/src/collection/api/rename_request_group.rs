...
impl Collection {
    async fn update_request_relative_path(&self, key: ResourceKey, old_prefix: &Path, new_prefix: &Path) -> Result<()> {
        ...
        let entry_relative_path_old = requests_lock.get(key)?.entry_relative_path.clone();
        let entry_relative_path_new = new_prefix.join((&entry_relative_path_old).strip_prefix(&old_prefix)?);
        requests_lock.get_mut(key)?.entry_relative_path = entry_relative_path_new.clone();
        ...
        let keys_to_rename =
            requests_lock
                ...
                .filter_map(|(key, iter_slot)| {
                    if iter_slot.value().entry_relative_path.starts_with(&group_relative_path_old) {
                        Some(key)
                    } else {
                        None
                    }
