...
pub struct EnvironmentServiceFs {
    // For Workspace and Project, this point to their environments folder
    // For App, this points to their workspaces folder
    base_path: PathBuf,
    fs: Arc<dyn FileSystem>,
}

impl EnvironmentServiceFs {
    pub fn new(base_path: PathBuf, fs: Arc<dyn FileSystem>) -> Self {
        Self { base_path, fs }
    }
}
...
impl EnvironmentServiceFsPort for EnvironmentServiceFs {
    ...
    async fn lookup_environments(
        ...
        lookup_source(self, ctx, &self.base_path, &mut environments).await?;
    ...
    async fn read_environment_sourcefile(
        ...
        let abs_path = self.base_path.join(format_env_file_name(id));
        let rdr = self
            .fs
    ...
    async fn create_environment(
        ...
        let abs_path = self.base_path.join(&file_name);
        if abs_path.exists() {
    ...
    async fn create_workspace_environment(
        ...
        // The base path points to workspaces/ folder
        let abs_path = self
            .base_path
