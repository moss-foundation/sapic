...
const ONBOARDING_WINDOW_ENTRY_POINT: &str = "onboarding.html";

/// Onboarding window controller
#[derive(Deref)]
pub struct OnboardingWindow<R: AppRuntime> {
    #[deref]
...
impl<R: AppRuntime> OnboardingWindow<R> {
    pub async fn new(delegate: &AppDelegate<R>) -> joinerror::Result<Self> {
        ...
        let win_builder = tauri::WebviewWindowBuilder::new(
            ...
            tauri::WebviewUrl::App(ONBOARDING_WINDOW_ENTRY_POINT.into()),
        )
...
#[async_trait]
impl<R: AppRuntime> AppWindowApi for OnboardingWindow<R> {
