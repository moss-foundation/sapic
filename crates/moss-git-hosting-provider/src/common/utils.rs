...
pub(crate) fn receive_auth_code(listener: &TcpListener) -> Result<(AuthorizationCode, CsrfToken)> {
    let mut stream = listener
    .incoming()
    .flatten()
    .next()
    .ok_or_else(|| anyhow!("Failed to perform initial credentials setup: listener terminated without accepting a connection"))?;
    ...
    let redirect_url = request_line.split_whitespace().nth(1).ok_or_else(|| {
        anyhow!("Failed to perform initial credentials setup: invalid request format")
    })?;
