...
mod tests {
    ...
    fn test_plugin_hello() -> Result<()> {
        ...
        let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .parent()
            .unwrap();
        let wasm_path = workspace_root.join("target/wasm32-wasip2/release/plugin_hello.wasm");
        ...
        let result = plugin.call_run(&mut store, "World")?;

        println!("Plugin said: {}", result);
        assert_eq!(result, "Hello, World from plugin!");
    ...
    mod async_test {
        ...
        #[tokio::test]
        async fn test_plugin_hello_async() -> Result<()> {
            let mut config = Config::new();
            config.wasm_component_model(true);
            ...
            let result = plugin.call_run(&mut store, "World").await?;

            println!("Plugin said (async): {}", result);
            assert_eq!(result, "Hello, World from plugin!");
