...
const ButtonLabel = ({
  ...
  if (isEditing && editable) {
    return (
      <input
        ...
        onBlur={handleBlur}
        className="text-md background-(--moss-controls-background) h-[18px] min-w-[80px] px-1 text-(--moss-controls-foreground) outline-none"
      />
    );
  }
  ...
  return (
    <span
      ref={spanRef}
      className={cn("text-md truncate text-left text-(--moss-controls-foreground)", className)}
      onDoubleClick={editable ? onStartEdit : undefined}
    >
      {title}
    </span>
