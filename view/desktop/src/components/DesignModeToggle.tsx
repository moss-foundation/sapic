...
export const DesignModeToggle = ({ className, disabled = true }: DesignModeToggleProps) => {
  ...
  return (
    <div
      className={cn(
        "background-(--moss-display-mode-background) @container/mode-toggle grid w-full grid-cols-2 items-center rounded-sm border-1 border-(--moss-display-mode-border) p-px",
        className
      )}
    >
      <div className={cn("")}>
        ...
        <label
          htmlFor="live"
          className={cn(
            "flex min-w-0 cursor-pointer items-center justify-center rounded-sm px-1 py-px leading-5 text-(--moss-display-mode-foreground) transition-colors duration-300 peer-checked:bg-white peer-checked:text-(--moss-display-mode-foreground-selected)"
          )}
        >
