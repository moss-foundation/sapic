...
export const SidebarEdgeHandler = ({ position, onClick, activityBarOffset = 0 }: SidebarEdgeHandlerProps) => {
  ...
  return (
    ...
    >
      <div
        className={`background-(--moss-primary) absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-10`}
      />

      <div
        className={`absolute ${isLeft ? "left-0.5" : "right-0.5"} background-(--moss-primary) top-1/2 h-24 w-1 -translate-y-1/2 rounded-full opacity-30 transition-all duration-200 group-hover:h-32 group-hover:w-1.5 group-hover:opacity-70`}
      />
      ...
      >
        <span
          className={`bg-opacity-80 background-(--moss-primary) flex h-6 w-6 items-center justify-center rounded-full text-white`}
        >
          {isLeft ? (
