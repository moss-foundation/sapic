  ...
  return (
    <SelectContext.Provider value={{ variant: contextVariant }}>
      <SelectPrimitive.Content
        ...
        ref={forwardedRef}
        className={cn(
          `background-(--moss-controls-background) border-(--moss-controls-border) z-50 w-56 rounded-lg border px-1.5 py-1.5 shadow-lg`,
          className
        )}
      >
...
const selectItemStyles = cva(
  `data-[highlighted]:background-(--moss-controls-background-hover) data-[state=checked]:background-(--moss-accent-secondary) data-[disabled]:grayscale-100 relative flex min-w-0 select-none items-center gap-1.5 rounded py-0.5 pl-[7px] pr-2 leading-5 outline-none data-[disabled]:cursor-not-allowed data-[highlighted]:cursor-pointer`
);
