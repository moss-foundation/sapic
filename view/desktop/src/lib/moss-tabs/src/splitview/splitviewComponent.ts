 ...
 */
export class SplitviewComponent extends Resizable implements ISplitviewComponent {
  ...
  set splitview(value: Splitview) {
    ...
    this._splitviewChangeDisposable.value = new CompositeDisposable(
      ...
      this._splitview.onDidRemoveView((e) => this._onDidRemoveView.fire(e))
    );
  }
  ...
  layout(width: number, height: number): void {
    const [size, orthogonalSize] =
      this.splitview.orientation === Orientation.HORIZONTAL ? [width, height] : [height, width];
  ...
  toJSON(): SerializedSplitview {
    const views: SerializedSplitviewPanel[] = this.splitview.getViews<SplitviewPanel>().map((view, i) => {
