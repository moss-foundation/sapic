...
describe("splitview", () => {
  ...
  test("high priority", () => {
    ...
    const view2 = new Testview(50, Number.POSITIVE_INFINITY, LayoutPriority.High);
  ...
  test("low priority", () => {
    ...
    const view1 = new Testview(50, Number.POSITIVE_INFINITY, LayoutPriority.Low);
  ...
  test("from descriptor", () => {
    ...
    expect([descriptor.views[0].size, descriptor.views[1].size, descriptor.views[2].size]).toEqual([80, 100, 120]);
  ...
  test("dnd: pointer events to move sash", () => {
    ...
    fireEvent(sashElement, new MouseEvent("pointerdown", { clientX: 50, clientY: 100 }));
    ...
    fireEvent(document, new MouseEvent("pointermove", { clientX: 70, clientY: 110 }));
    expect([view1.size, view2.size]).toEqual([220, 180]);

    // expect a delta move of 75 - 70 = 5
    fireEvent(document, new MouseEvent("pointermove", { clientX: 75, clientY: 110 }));
    expect([view1.size, view2.size]).toEqual([225, 175]);

    // end the drag event
    fireEvent(document, new MouseEvent("pointerup", { clientX: 70, clientY: 110 }));
    ...
    fireEvent(document, new MouseEvent("pointermove", { clientX: 100, clientY: 100 }));
  ...
  test("that margins are applied to view sizing", () => {
    ...
    expect([view1.size, view2.size, view3.size, view4.size]).toEqual([213, 213, 213, 213]); // 213 + 24 + 213 + 24 + 213 + 24 + 213 = 924

    let viewQuery = Array.from(
      container.querySelectorAll(".dv-split-view-container > .dv-view-container > .dv-view").entries()
    )
    ...
    let sashQuery = Array.from(
      container.querySelectorAll(".dv-split-view-container > .dv-sash-container > .dv-sash").entries()
    )
      .map(([i, e]) => e as HTMLElement)
      .map((e) => ({
        left: e.style.left,
