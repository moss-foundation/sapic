...
class MockGridview implements IGridView {
  ...
  onDidChange: Event<IViewSize | undefined> = new Emitter<IViewSize | undefined>().event;
...
describe("gridview", () => {
  ...
  test("gridview nested proportional layouts", () => {
    const gridview = new Gridview(true, { separatorBorder: "" }, Orientation.HORIZONTAL);
    gridview.layout(1000, 1000);
  ...
  test("visibility check", () => {
    ...
    function assertVisibility(visibility: boolean[]) {
      expect(gridview.isViewVisible(getGridLocation(view1.element))).toBe(visibility[0]);
      expect(gridview.isViewVisible(getGridLocation(view2.element))).toBe(visibility[1]);
      expect(gridview.isViewVisible(getGridLocation(view3.element))).toBe(visibility[2]);
      expect(gridview.isViewVisible(getGridLocation(view4.element))).toBe(visibility[3]);
      expect(gridview.isViewVisible(getGridLocation(view5.element))).toBe(visibility[4]);
      expect(gridview.isViewVisible(getGridLocation(view6.element))).toBe(visibility[5]);
    }
