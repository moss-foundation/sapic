...
describe("dockviewComponent", () => {
  ...
  describe("memory leakage", () => {
    ...
    test("event leakage", async () => {
      ...
      if (Emitter.MEMORY_LEAK_WATCHER.size > 0) {
        console.warn(`${Emitter.MEMORY_LEAK_WATCHER.size} undisposed resources`);

        for (const entry of Array.from(Emitter.MEMORY_LEAK_WATCHER.events)) {
          console.log("disposal", entry[1]);
        }
        throw new Error(`${Emitter.MEMORY_LEAK_WATCHER.size} undisposed resources`);
      }
  ...
  test("that deserializing an empty layout has zero groups and a watermark", () => {
    ...
    expect(dockview.element.querySelectorAll(".dv-watermark-container").length).toBe(1);
  ...
  test("that moving the last panel to be floating should leave an empty gridview", () => {
    ...
    expect(dockview.element.querySelectorAll(".dv-view-container > .dv-view").length).toBe(1);

    dockview.addFloatingGroup(panel1);

    expect(dockview.element.querySelectorAll(".dv-view-container > .dv-view").length).toBe(0);
  });
  ...
  describe("popout group", () => {
    ...
    test("that panel is rendered when moving from popout to new group", async () => {
      ...
      expect(panel2.group.element.querySelectorAll(".dv-content-container > .testpanel-panel_2").length).toBe(1);
      ...
      expect(dockview.element.querySelectorAll(".dv-render-overlay > .testpanel-panel_3").length).toBe(1);
      expect(panel2.group.element.querySelectorAll(".dv-content-container > .testpanel-panel_3").length).toBe(0);
    });
