...
export const toggleClass = (element: HTMLElement, className: string, isToggled: boolean): void => {
...
class FocusTracker extends CompositeDisposable implements IFocusTracker {
  ...
  constructor(element: HTMLElement | Window) {
    ...
    this._refreshStateHandler = () => {
      const currentNodeHasFocus = isAncestor(document.activeElement, <HTMLElement>element);
...
export function disableIframePointEvents() {
  const iframes: HTMLElement[] = [...getElementsByTagName("iframe"), ...getElementsByTagName("webview")];
