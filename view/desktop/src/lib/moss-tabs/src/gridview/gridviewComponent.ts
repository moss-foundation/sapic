...
export class GridviewComponent extends BaseGrid<GridviewPanel> implements IGridviewComponent {
  ...
  readonly onDidRemoveGroup: Event<GridviewPanel> = this._onDidRemoveGroup.event;
  ...
  private readonly _onDidActiveGroupChange = new Emitter<GridviewPanel | undefined>();
  readonly onDidActiveGroupChange: Event<GridviewPanel | undefined> = this._onDidActiveGroupChange.event;
  ...
  override updateOptions(options: Partial<GridviewComponentOptions>): void {
    ...
    const hasOrientationChanged =
      typeof options.orientation === "string" && this.gridview.orientation !== options.orientation;
  ...
  movePanel(panel: GridviewPanel, options: { direction: Direction; reference: string; size?: number }): void {
  ...
  public addPanel<T extends object = Parameters>(options: AddComponentOptions<T>): IGridviewPanel {
    let relativeLocation: number[] = options.location ?? [0];

    if (options.position?.referencePanel) {
      const referenceGroup = this._groups.get(options.position.referencePanel)?.value;
  ...
  public moveGroup(referenceGroup: IGridPanelComponentView, groupId: string, target: Position): void {
