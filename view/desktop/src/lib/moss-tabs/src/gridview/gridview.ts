...
function flipNode<T extends Node>(node: T, size: number, orthogonalSize: number): T {
  if (node instanceof BranchNode) {
    ...
    return new LeafNode(node.view, orthogonal(node.orientation), orthogonalSize) as T;
  }
}
...
export function getRelativeLocation(rootOrientation: Orientation, location: number[], direction: Position): number[] {
...
export function getLocationOrientation(rootOrientation: Orientation, location: number[]): Orientation {
...
export function isGridBranchNode<T extends IGridView>(node: GridNode<T>): node is GridBranchNode<T> {
...
export class Gridview implements IDisposable {
  ...
  private _margin = 0;
  private _maximizedNode: { leaf: LeafNode; hiddenOnMaximize: LeafNode[] } | undefined = undefined;
  ...
  readonly onDidChange: Event<{ size?: number; orthogonalSize?: number }> = this._onDidChange.event;
  ...
  private readonly _onDidMaximizedNodeChange = new Emitter<MaximizedViewChanged>();
  ...
  public deserialize<T>(json: SerializedGridview<T>, deserializer: IViewDeserializer): void {
    const orientation = json.orientation;
    const height = orientation === Orientation.VERTICAL ? json.height : json.width;

    this._deserialize(json.root as ISerializedBranchNode, orientation, deserializer, height);
  ...
  private _getViews(node: Node, orientation: Orientation, cachedVisibleSize?: number): GridNode<IGridView> {
  ...
  public addView(view: IGridView, size: number | Sizing, location: number[]): void {
    if (this.hasMaximizedView()) {
      this.exitMaximizedView();
    }
