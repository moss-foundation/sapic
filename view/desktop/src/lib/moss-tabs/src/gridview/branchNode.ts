...
export class BranchNode extends CompositeDisposable implements IView {
      ...
      const descriptor = {
        views: childDescriptors.map((childDescriptor) => {
          return {
            ...
            visible:
              childDescriptor.node instanceof LeafNode && childDescriptor.visible !== undefined
  ...
  public addChild(node: Node, size: number | Sizing, index: number, skipLayout?: boolean): void {
    if (index < 0 || index > this.children.length) {
      throw new Error("Invalid index");
    }
  ...
  private setupChildrenEvents(): void {
    ...
    this._childrenDisposable = new CompositeDisposable(
      Event.any(...this.children.map((c) => c.onDidChange))((e) => {
        /**
         * indicate a change has occurred to allows any re-rendering but don't bubble
