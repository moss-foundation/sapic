...
import { CompositeDisposable, IDisposable, IValueDisposable } from "../../../lifecycle";
...
export class TabsContainer extends CompositeDisposable implements ITabsContainer {
  ...
  public openPanel(panel: IDockviewPanel, index: number = this.tabs.length): void {
  ...
  private addTab(tab: IValueDisposable<Tab>, index: number = this.tabs.length): void {
