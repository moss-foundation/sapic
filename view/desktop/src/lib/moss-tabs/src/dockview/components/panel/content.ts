...
export class ContentContainer extends CompositeDisposable implements IContentContainer {
      ...
      canDisplayOverlay: (event, position) => {
        ...
        if (data && data.viewId === this.accessor.id) {
          ...
          const groupHasOnePanelAndIsActiveDragElement =
            this.group.panels.length === 1 && data.groupId === this.group.id;
  ...
  renderPanel(panel: IDockviewPanel, options: { asActive: boolean } = { asActive: true }): void {
  ...
  public closePanel(): void {
    if (this.panel) {
      if (this.panel.api.renderer === "onlyWhenVisible") {
        this.panel.view.content.element.parentElement?.removeChild(this.panel.view.content.element);
      }
    }
