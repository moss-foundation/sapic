...
function moveGroupWithoutDestroying(options: { from: DockviewGroupPanel; to: DockviewGroupPanel }) {
...
export class DockviewComponent extends BaseGrid<DockviewGroupPanel> implements IDockviewComponent {
  ...
  readonly onWillDragGroup: Event<GroupDragEvent> = this._onWillDragGroup.event;
  ...
  private readonly _onWillShowOverlay = new Emitter<WillShowOverlayLocationEvent>();
  readonly onWillShowOverlay: Event<WillShowOverlayLocationEvent> = this._onWillShowOverlay.event;

  private readonly _onUnhandledDragOverEvent = new Emitter<DockviewDndOverlayEvent>();
  readonly onUnhandledDragOverEvent: Event<DockviewDndOverlayEvent> = this._onUnhandledDragOverEvent.event;

  private readonly _onDidRemovePanel = new Emitter<IDockviewPanel>();
  readonly onDidRemovePanel: Event<IDockviewPanel> = this._onDidRemovePanel.event;
  ...
  private readonly _onDidActivePanelChange = new Emitter<IDockviewPanel | undefined>();
  readonly onDidActivePanelChange: Event<IDockviewPanel | undefined> = this._onDidActivePanelChange.event;

  private readonly _onDidMovePanel = new Emitter<MovePanelEvent>();
  readonly onDidMovePanel = this._onDidMovePanel.event;

  private readonly _onDidMaximizedGroupChange = new Emitter<DockviewMaximizedGroupChanged>();
  ...
  readonly onDidRemoveGroup: Event<DockviewGroupPanel> = this._onDidRemoveGroup.event;

  protected readonly _onDidAddGroup = new Emitter<DockviewGroupPanel>();
  readonly onDidAddGroup: Event<DockviewGroupPanel> = this._onDidAddGroup.event;

  private readonly _onDidActiveGroupChange = new Emitter<DockviewGroupPanel | undefined>();
  readonly onDidActiveGroupChange: Event<DockviewGroupPanel | undefined> = this._onDidActiveGroupChange.event;
  ...
  constructor(container: HTMLElement, options: DockviewComponentOptions) {
    ...
    this._rootDropTarget = new Droptarget(this.element, {
      ...
      overlayModel: this.options.rootOverlayModel ?? DEFAULT_ROOT_OVERLAY_MODEL,
    });
  ...
  addPopoutGroup(
    ...
    const groupId = options?.overridePopoutGroup?.id ?? this.getNextGroupId();

    const _window = new PopoutWindow(
      `${this.id}-${groupId}`, // unique id
      theme ?? "",
      {
        url: options?.popoutUrl ?? this.options?.popoutUrl ?? "/popout.html",
  ...
  addFloatingGroup(item: DockviewPanel | DockviewGroupPanel, options?: FloatingGroupOptionsInternal): void {
    ...
    if (item instanceof DockviewPanel) {
      ...
      const popoutReferenceGroupId = this._popoutGroups.find((_) => _.popoutGroup === group)?.referenceGroup;
      const popoutReferenceGroup = popoutReferenceGroupId ? this.getPanel(popoutReferenceGroupId) : undefined;

      const skip = typeof options?.skipRemoveGroup === "boolean" && options.skipRemoveGroup;
    ...
    function getAnchoredBox(): AnchoredBox {
      ...
      return {
        left: typeof options?.x === "number" ? Math.max(options.x, 0) : DEFAULT_FLOATING_GROUP_POSITION.left,
        top: typeof options?.y === "number" ? Math.max(options.y, 0) : DEFAULT_FLOATING_GROUP_POSITION.top,
        width: typeof options?.width === "number" ? Math.max(options.width, 0) : DEFAULT_FLOATING_GROUP_POSITION.width,
        height:
          typeof options?.height === "number" ? Math.max(options.height, 0) : DEFAULT_FLOATING_GROUP_POSITION.height,
      };
    }
    ...
    const overlay = new Overlay({
      ...
      minimumInViewportWidth:
        this.options.floatingGroupBounds === "boundedWithinViewport"
          ? undefined
          : (this.options.floatingGroupBounds?.minimumWidthWithinViewport ?? DEFAULT_FLOATING_GROUP_OVERFLOW_SIZE),
      minimumInViewportHeight:
        this.options.floatingGroupBounds === "boundedWithinViewport"
          ? undefined
          : (this.options.floatingGroupBounds?.minimumHeightWithinViewport ?? DEFAULT_FLOATING_GROUP_OVERFLOW_SIZE),
    });
    ...
    overlay.setupDrag(<HTMLElement>el, {
      inDragMode: typeof options?.inDragMode === "boolean" ? options.inDragMode : false,
    });
  ...
  override updateOptions(options: Partial<DockviewComponentOptions>): void {
      ...
      for (const group of this._floatingGroups) {
        switch (options.floatingGroupBounds) {
          ...
          case undefined:
            ...
            break;
          default:
            group.overlay.minimumInViewportHeight = options.floatingGroupBounds?.minimumHeightWithinViewport;
            group.overlay.minimumInViewportWidth = options.floatingGroupBounds?.minimumWidthWithinViewport;
        }
    ...
    if ("rootOverlayModel" in options) {
      this._rootDropTarget.setOverlayModel(options.rootOverlayModel ?? DEFAULT_ROOT_OVERLAY_MODEL);
    }
  ...
  override layout(width: number, height: number, forceResize?: boolean | undefined): void {
  ...
  toJSON(): SerializedDockview {
    ...
    const floats: SerializedFloatingGroup[] = this._floatingGroups.map((group) => {
      return {
    ...
    const popoutGroups: SerializedPopoutGroup[] = this._popoutGroups.map((group) => {
      return {
  ...
  fromJSON(data: SerializedDockview): void {
    ...
    try {
      ...
      for (const serializedPopoutGroup of serializedPopoutGroups) {
        ...
        this.addPopoutGroup((gridReferenceGroup ? this.getPanel(gridReferenceGroup) : undefined) ?? group, {
  ...
  addPanel<T extends object = Parameters>(options: AddPanelOptions<T>): DockviewPanel {
      ...
      if (isPanelOptionsWithPanel(options.position)) {
        ...
        referenceGroup =
          typeof options.position.referenceGroup === "string"
            ? this._groups.get(options.position.referenceGroup)?.value
        ...
        const group = this.orthogonalize(directionToPosition(<Direction>options.position.direction));
    ...
    if (referenceGroup) {
      const target = toTarget(<Direction>options.position?.direction || "within");
      ...
      if (options.floating) {
        ...
        const floatingGroupOptions =
          typeof options.floating === "object" && options.floating !== null ? options.floating : {};
        ...
        const group = this.createGroupAtLocation(
          relativeLocation,
          this.orientationAtLocation(relativeLocation) === Orientation.VERTICAL ? initial?.height : initial?.width
        );
        panel = this.createPanel(options, group);
        group.model.openPanel(panel, {
          skipSetActive: options.inactive,
      ...
      const group = this.createGroupAtLocation(
        [0],
        this.gridview.orientation === Orientation.VERTICAL ? initial?.height : initial?.width
      );
      panel = this.createPanel(options, group);
      group.model.openPanel(panel, {
        skipSetActive: options.inactive,
  ...
  addGroup(options?: AddGroupOptions): DockviewGroupPanel {
    if (options) {
      ...
      if (isGroupOptionsWithPanel(options)) {
        ...
        const group = this.orthogonalize(directionToPosition(<Direction>options.direction));
      ...
      const size =
        this.getLocationOrientation(relativeLocation) === Orientation.VERTICAL
  ...
  private getLocationOrientation(location: number[]) {
    return location.length % 2 == 0 && this.gridview.orientation === Orientation.HORIZONTAL
  ...
  moveGroupOrPanel(options: MoveGroupOrPanelOptions): void {
    ...
    const sourceGroup = sourceGroupId ? this._groups.get(sourceGroupId)?.value : undefined;
    ...
    if (!destinationTarget || destinationTarget === "center") {
      ...
      if (sourceGroup.size < 2) {
        ...
        if (sourceGroup.api.location.type === "popout") {
          ...
          const removedPanel: IDockviewPanel | undefined = this.movingLock(() =>
            popoutGroup.popoutGroup.model.removePanel(popoutGroup.popoutGroup.panels[0], {
              skipSetActive: true,
              skipSetActiveGroup: true,
            })
          );
  ...
  createGroup(options?: GroupOptions): DockviewGroupPanel {
    ...
    if (id && this._groups.has(options.id!)) {
      console.warn(`dockview: Duplicate group id ${options?.id}. reassigning group id to avoid errors`);
      id = undefined;
    }
  ...
  private createPanel(options: AddPanelOptions, group: DockviewGroupPanel): DockviewPanel {
  ...
  private createGroupAtLocation(location: number[], size?: number): DockviewGroupPanel {
    const group = this.createGroup();
  ...
  private findGroup(panel: IDockviewPanel): DockviewGroupPanel | undefined {
    return Array.from(this._groups.values()).find((group) => group.value.model.containsPanel(panel))?.value;
  }
