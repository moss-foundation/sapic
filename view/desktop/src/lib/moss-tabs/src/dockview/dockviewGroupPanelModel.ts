...
export class DockviewGroupPanelModel extends CompositeDisposable implements IDockviewGroupPanelModel {
  ...
  readonly onGroupDragStart: Event<GroupDragEvent> = this._onGroupDragStart.event;

  private readonly _onDidAddPanel = new Emitter<DockviewGroupChangeEvent>();
  readonly onDidAddPanel: Event<DockviewGroupChangeEvent> = this._onDidAddPanel.event;

  private readonly _onDidPanelTitleChange = new Emitter<TitleEvent>();
  readonly onDidPanelTitleChange: Event<TitleEvent> = this._onDidPanelTitleChange.event;

  private readonly _onDidPanelParametersChange = new Emitter<Parameters>();
  readonly onDidPanelParametersChange: Event<Parameters> = this._onDidPanelParametersChange.event;

  private readonly _onDidRemovePanel = new Emitter<DockviewGroupChangeEvent>();
  readonly onDidRemovePanel: Event<DockviewGroupChangeEvent> = this._onDidRemovePanel.event;

  private readonly _onDidActivePanelChange = new Emitter<DockviewGroupChangeEvent>();
  readonly onDidActivePanelChange: Event<DockviewGroupChangeEvent> = this._onDidActivePanelChange.event;
  ...
  get renderContainer(): OverlayRenderContainer {
    return this._overwriteRenderContainer ?? this.accessor.overlayRenderContainer;
  }
  ...
  public moveToNext(options?: { panel?: IDockviewPanel; suppressRoll?: boolean }): void {
    if (!options) {
      options = {};
    }
    if (!options.panel) {
  ...
  public moveToPrevious(options?: { panel?: IDockviewPanel; suppressRoll?: boolean }): void {
    if (!options) {
      options = {};
    }
    if (!options.panel) {
  ...
  private doClose(panel: IDockviewPanel): void {
    ...
    this.accessor.removePanel(
      panel,
      isLast && this.accessor.options.noPanelsOverlay === "emptyGroup" ? { removeEmptyGroup: false } : undefined
    );
  }
  ...
  private handleDropEvent(type: "header" | "content", event: DragEvent, position: Position, index?: number): void {
