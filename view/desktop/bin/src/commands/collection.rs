...
use moss_workbench::{models::operations::OpenWorkspaceInput, workbench::Workbench};
use moss_workspace::models::operations::OpenCollectionInput;
...
pub async fn example_index_collection_command(
    ...
    let workspace_manager = app_manager
        .services()
        .get_by_type::<Workbench<tauri::Wry>>(&app_handle)
        .await?;
    ...
    let current_workspace = workspace_manager.active_workspace().unwrap();
    ...
    let collections = current_workspace.collections().await.unwrap();
